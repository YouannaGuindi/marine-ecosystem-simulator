...
public class PotentialThreats {
	...
	public PotentialThreats(double temp, double salinity, double depth, String ebb, MarineOrganism[] scannedOrganisms,
	...
	public String findthreat(SubThreatsHuman h, SubThreatsNatural n) {
	    // Ensure the objects passed are of the correct types
	    if (!(h instanceof SubHumanPollution) || !(h instanceof SubHumanOverfishing)) {
	        throw new IllegalArgumentException("Invalid SubThreatsHuman type passed to findthreat method.");
	    }
	    if (!(n instanceof SubNaturalWaterConditions)) {
	        throw new IllegalArgumentException("Invalid SubThreatsNatural type passed to findthreat method.");
	    }

	    SubHumanPollution humanPollution = (SubHumanPollution) h;
	    SubHumanOverfishing humanOverfishing = (SubHumanOverfishing) h;
	    SubNaturalWaterConditions waterConditions = (SubNaturalWaterConditions) n;

	    StringBuilder threatsSummary = new StringBuilder();
	    try {
	        // Aggregate human threats
	        threatsSummary.append(h.findHumanThreat(humanPollution, humanOverfishing));
	        // Aggregate natural threats
	        threatsSummary.append(n.findNaturalThreat(waterConditions));
	    } catch (Exception e) {
	        System.out.println("Error evaluating threats: " + e.getMessage());
	    }
	    return threatsSummary.toString();
	}
}
